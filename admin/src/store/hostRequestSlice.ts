...
export const getHostRequest: any = createAsyncThunk(
  "api/admin/host/fetchHostRequest",
  async (payload: AllUsersPayload | undefined) => {
    return apiInstanceFetch.get(`api/admin/host/fetchHostRequest?status=${payload?.status}&start=${payload?.start}&limit=${payload?.limit}&search=${payload?.search}`);
  }
);
...
export const hostRequestUpdate: any = createAsyncThunk(
  "api/admin/host/handleHostRequest?status=2",
  async (payload: AllUsersPayload | undefined) => {
    return await apiInstanceFetch.patch(
      `api/admin/host/handleHostRequest?status=2&requestId=${payload?.requestId}&userId=${payload?.userId}`
    );
  }
);

export const hostRequestDeclined: any = createAsyncThunk(
  "api/admin/host/handleHostRequest?status=3",
  async (payload: AllUsersPayload | undefined) => {
    return await apiInstanceFetch.patch(
      `api/admin/host/handleHostRequest?status=3&requestId=${payload?.requestId}&userId=${payload?.userId}&reason=${payload?.reason}`
    );
  }
);
...
export const assignToExpertAccepted: any = createAsyncThunk(
  ...
  async (payload: any) => {
    ...
    return apiInstanceFetch.patch(
      `api/admin/host/assignHostToAgency?agencyId=${payload?.agencyId}&requestId=${payload?.requestId}&userId=${payload?.userId}`
    );
  }
);
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<any>) => {
        ...
        state.hostRequest = action.payload.data;
        state.totalHostRequest = action.payload.total
      }
    );
    ...
    builder.addCase(assignToExpertAccepted.fulfilled, (state, action: any) => {
      ...
      if (action.payload.status) {
        ...
        state.hostRequest = state.hostRequest.filter((request) =>
...
export const { setHostRequest } = hostRequestSlice.actions;
